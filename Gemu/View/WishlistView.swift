    ...
    var body: some View {
        ZStack {
            VStack {
                ...
                List {
                    ForEach(viewModel.fetchedResultsGame.fetchedObjects?.filter { search != "" ? $0.name?.lowercased().contains(search.lowercased()) ?? false && ($0.lists?.collection == true) : ($0.lists?.wishlist == true) } ?? [], id: \.self) { game in
...
struct WishlistView_Previews: PreviewProvider {
    static var previews: some View {
        WishlistView(viewModel: GemuViewModel(), showDetail: .constant(true))
    }
}
